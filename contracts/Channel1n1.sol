...
contract Channel1n1 is ECRecovery {
    ...
    function joinChannel(uint256 _channelId) public payable {
        require(_channelId < channels.length, "ChannelID does not exist");
        require(
    ...
    function closeChannel(
        ...
        require(
            _status == 3,
            "The car needs to have been released before closing a channel"
        );
        require(
            channel.car1 == msg.sender || channel.car2 == msg.sender,
            "Only a participant is allowed to close this Channel"
        );
        require(!channel.closed, "This channel is closed already");
        require(
            msg.sender != channel.firstCloseApproval,
            "This participant already closed the channel and requires the opponent to close as well"
        );

        address _secondCar = (channel.car1 == msg.sender)
            ? channel.car2
            : channel.car1;
        ...
        bytes32 _hashKeccak = keccak256(
            abi.encodePacked("\x19Ethereum Signed Message:\n32", _hash)
        );

        require(
            recover(_hashKeccak, _sig) == _secondCar,
        ...
        if (channel.firstCloseApproval != address(0)) {
            ...
            // the first time close is called on this channel
